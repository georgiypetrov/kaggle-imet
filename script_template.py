...
for model in models:
    for fold in params[model]['folds']:
        ...
        run(f'python -m imet.main predict_test {run_root}model_{model}_fold_{fold} --model {model} --input-size {input_size} --batch-size {batch_size} --tta {tta}')
